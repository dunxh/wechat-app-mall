...
Page({
  data: {
    ...
    imgUrls: null,
    baseUrl:"https://39.99.169.162:8085/",
    indicatorDots: true,
    autoplay: true,
    interval: 5000,
    duration: 1000,
  ...
  imgsc:function(e){
    wx.chooseImage({
      ...
      success: function (res) {
        ...
        wx.uploadFile({
          url: 'https://39.99.169.162:8085/api/v1/admin/uploadBanner',      //此处换上你的接口地址
  ...
  getList: function(){
    app.http('/home/content', { page: this.data.page})
      .then(res => {
        if (res.code == 200 && res.data.hotProductList.length > 0) {
          this.data.page++
          let list = this.data.hotProductList
          for (let i = 0; i < res.data.hotProductList.length; i++) {
            list.push(res.data.hotProductList[i])
          }
          this.setData({
            list: list,
            page: this.data.page
